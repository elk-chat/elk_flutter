...
Map<dynamic, dynamic> decodeProtobuf(dynamic msgData) {
  ...
  if (sig == SigMapper['SIG_ERROR']) {
    ...
    decodeProto =
        (buffer) => proto.RequestVerificationCodeReq.fromBuffer(buffer);
  } else if (sig == SigMapper['SIG_REQUEST_VERIFICATION_CODE_RESP']) {
    decodeMethod = 'SIG_REQUEST_VERIFICATION_CODE_RESP';
    decodeProto =
        (buffer) => proto.RequestVerificationCodeResp.fromBuffer(buffer);
    ...
    decodeProto =
        (buffer) => proto.UserGetChatUserSuperscriptReq.fromBuffer(buffer);
  } else if (sig == SigMapper['SIG_USER_GET_CHAT_USER_SUPERSCRIPT_RESP']) {
    decodeMethod = 'SIG_USER_GET_CHAT_USER_SUPERSCRIPT_RESP';
    decodeProto =
        (buffer) => proto.UserGetChatUserSuperscriptResp.fromBuffer(buffer);
    ...
    decodeProto =
        (buffer) => proto.UtilityNewMultipartUploadReq.fromBuffer(buffer);
  } else if (sig == SigMapper['SIG_UTILITY_NEW_MULTIPART_UPLOAD_RESP']) {
    decodeMethod = 'SIG_UTILITY_NEW_MULTIPART_UPLOAD_RESP';
    decodeProto =
        (buffer) => proto.UtilityNewMultipartUploadResp.fromBuffer(buffer);
    ...
    decodeProto =
        (buffer) => proto.UtilityCompleteMultipartUploadReq.fromBuffer(buffer);
  } else if (sig == SigMapper['SIG_UTILITY_COMPLETE_MULTIPART_UPLOAD_RESP']) {
    decodeMethod = 'SIG_UTILITY_COMPLETE_MULTIPART_UPLOAD_RESP';
    decodeProto =
        (buffer) => proto.UtilityCompleteMultipartUploadResp.fromBuffer(buffer);
